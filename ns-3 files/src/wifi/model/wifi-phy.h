...
public:
   ...
   * \param mpdutype the type of the MPDU as defined in WifiPhy::MpduType.
   */
  void SendPacket (Ptr<const Packet> packet, WifiTxVector txVector, Time frameDuration, MpduType mpdutype = NORMAL_MPDU);
  /**
   * \param packet the packet to send
   * \param txVector the TXVECTOR that has tx parameters such as mode, the transmission mode to use to send
   *        this packet, and txPowerLevel, a power level to use to send this packet. The real transmission
  ...
  */
   ...
   */
  virtual std::vector<uint16_t> GetSupportedChannelWidthSet (void) const;
  /**
  ...
  // Yubing
  std::string GetMostMcs (void);
  double GetRss (void);
  void SetRSSAndMCS (std::vector<std::pair<std::string, uint16_t>> MCS, std::pair<double,double> rss);
  std::pair<std::pair<Mac48Address, Mac48Address>, double> GetSensitivityMap (void);  
...
private:
  ...
  // Yubing
  std::vector<std::pair<std::string, uint16_t>> MCSHistory;
  std::pair<double,double> m_rssStas;
  double m_rxPowerDbm;
  std::pair<std::pair<Mac48Address, Mac48Address>, double> m_rxSensitivityMap;
};
