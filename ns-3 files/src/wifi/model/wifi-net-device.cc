...
bool
WifiNetDevice::Send (Ptr<Packet> p, const Address& dest, uint16_t protocolNumber)
{
  m_pktIdentifier++;
  NS_LOG_FUNCTION (this << m_pktIdentifier << p << dest << protocolNumber);
  NS_ASSERT (Mac48Address::IsMatchingType (dest));
  Ptr<Packet> packet = p->Copy ();
  //std::cout << "pkt: " << p << " " << packet << " identifier: " << m_pktIdentifier << std::endl;
...
WifiNetDevice::ForwardUp (Ptr<Packet> packet, Mac48Address from, Mac48Address to)
{
  ...
  //printf ("device: %p, Rx Device Now: %f, pkt pointer: %p \n", this , Simulator::Now ().GetSeconds (), packet);
  //std::cout << "pkt: " << packet << std::endl;
  LlcSnapHeader llc;
