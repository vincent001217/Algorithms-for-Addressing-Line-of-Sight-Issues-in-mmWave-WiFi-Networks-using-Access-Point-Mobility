...
private:
  ...
  // Yubing
  void DoSetDataMode (Mac48Address address, WifiMode mode) {m_mcsDataMap[address] = mode;};
  void DoSetControlMode (Mac48Address address, WifiMode mode) {m_mcsControlMap[address] = mode;};

  WifiMode m_dataMode; //!< Wifi mode for unicast DATA frames
  WifiMode m_ctlMode;  //!< Wifi mode for RTS frames
  // Yubing
  std::map<Mac48Address, WifiMode> m_mcsDataMap;
  std::map<Mac48Address, WifiMode> m_mcsControlMap;
};
