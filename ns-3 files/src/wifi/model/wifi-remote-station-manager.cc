...
{
  NS_LOG_FUNCTION (this << address << *header << packet << GetNonUnicastMode ());
...
//  v.SetPreambleType (GetPreambleForTransmission (mode, address));
  ...
  v.SetNess (0);
  v.SetStbc (false);
  NS_LOG_FUNCTION (this << address << *header << packet << v.GetMode ());
  return v;
}
...
// Yubing
void 
WifiRemoteStationManager::SetModeBasedOnRxSensitivity (Mac48Address address, WifiMode mode)
{
  NS_LOG_FUNCTION (this << address << " " << mode);
  DoSetDataMode(address, mode);
  DoSetControlMode(address, mode);
}
