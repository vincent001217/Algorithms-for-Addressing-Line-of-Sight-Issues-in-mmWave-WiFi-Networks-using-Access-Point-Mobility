...
// Yubing
void 
YansWifiChannel::SetChannelStatus (bool channelStatus)
{
  m_channelStatus = channelStatus;
}

void 
YansWifiChannel::SetScenarioModel (Ptr<Obstacle> scenario)
{
  m_scenario = scenario;
}
              ...
              if (m_experimentalMode)
              ...
              else
                {
                    rxPowerDbm = m_loss->CalcRxPower (txPowerDbm, senderMobility, receiverMobility) +
                               ...
                               (*i)->GetDirectionalAntenna ()->GetRxGainDbi (azimuthRx) + m_scenario->GetFadingInfo(sender_pos, receiverMobility->GetPosition ()).second;        // Receiver's antenna gain.
                  NS_LOG_DEBUG ("Tx antenna" << senderAnt->GetTxGainDbi (azimuthTx) << " paht loss " << m_loss->CalcRxPower (txPowerDbm, senderMobility, receiverMobility) << " rx antenna "<< ((*i)->GetDirectionalAntenna ()->GetRxGainDbi (azimuthRx)) << " fading " << m_scenario->GetFadingInfo(sender_pos, receiverMobility->GetPosition ()).second << " rxPowerDbm " << rxPowerDbm);
                }
