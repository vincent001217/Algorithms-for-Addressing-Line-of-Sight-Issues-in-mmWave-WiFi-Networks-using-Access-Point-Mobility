...
void
EdcaTxopN::CopyBlockAckAgreements (Mac48Address recipient, Ptr<EdcaTxopN> target)
{
...
EdcaTxopN::NotifyAccessGranted (void)
{
      ...
      if (m_currentPacket == 0)
        {
          ...
          NS_LOG_DEBUG ("dequeued size=" << m_currentPacket->GetSize () <<
                        ...
                        ", seq=" << m_currentHdr.GetSequenceControl () << ", m_currentPacket " << m_currentPacket << ", identifier " << sequence);
          if (m_currentHdr.IsQosData () && !m_currentHdr.GetAddr1 ().IsBroadcast ())
            {
              VerifyBlockAck ();
            }
  ...
  NS_LOG_FUNCTION (this << m_currentHdr.GetAddr1 () << m_currentHdr.GetAddr1 ().IsGroup ());
      ...
      else
        {
          ...
          if (m_currentHdr.IsQosData ()
              ...
              && m_msduAggregator != 0 && !m_currentHdr.IsRetry ())
            {
              ...
              NS_LOG_FUNCTION ("aggregate packet " << m_currentPacket);
              ...
              while (peekedItem != 0)
                {
                  m_pktIdentifier++;
                  NS_LOG_FUNCTION ("aggregate packet " << peekedItem->GetPacket ());
...
{
  NS_LOG_FUNCTION (this << (uint16_t)nMpdus << m_currentPacket << m_currentHdr << m_currentHdr.IsQosData ());
  if (m_currentPacket!= 0)
    {
  uint8_t tid = GetTid (m_currentPacket, m_currentHdr);
  if (GetAmpduExist (m_currentHdr.GetAddr1 ()))
