...
void
DcaTxop::SetTxOkCallback (TxPacketOk callback)
{
...
void
DcaTxop::SetTxOkNoAckCallback (TxOk callback)
{
  NS_LOG_FUNCTION (this << &callback);
...
{
  NS_LOG_FUNCTION (this << m_currentPacket << m_queue->HasPackets () << !m_dcf->IsAccessRequested () << m_manager->IsAccessAllowed ());
  if (m_currentPacket == 0
      && m_queue->HasPackets ()
      && !m_dcf->IsAccessRequested ()
      && m_manager->IsAccessAllowed ())
    {
      NS_LOG_FUNCTION (this << "accessed");
      m_manager->RequestAccess (m_dcf);
    }
}
  ...
  if (it != m_storedPackets.end ())
    {
      ...
      NS_LOG_FUNCTION (this << "m_currentPacket" << m_currentPacket);
    }
  ...
  if (m_currentPacket == 0)
    {
      ...
      m_fragmentNumber = 0;
      NS_LOG_DEBUG (m_currentPacket << "dequeued size=" << m_currentPacket->GetSize () <<
