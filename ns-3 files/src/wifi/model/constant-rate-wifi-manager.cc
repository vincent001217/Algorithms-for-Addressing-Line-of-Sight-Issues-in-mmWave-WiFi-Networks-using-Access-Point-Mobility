...
ConstantRateWifiManager::DoGetDataTxVector (WifiRemoteStation *st)
{
  ...
  // Yubing
  if (m_mcsDataMap.find(GetAddress (st)) == m_mcsDataMap.end()) {
  ...
  return WifiTxVector (m_mcsDataMap[GetAddress (st)], GetDefaultTxPowerLevel (), GetLongRetryCount (st), GetPreambleForTransmission (m_dataMode, GetAddress (st)), ConvertGuardIntervalToNanoSeconds (m_dataMode, GetShortGuardInterval (st), NanoSeconds (GetGuardInterval (st))), GetNumberOfAntennas (), Min (GetMaxNumberOfTransmitStreams (), GetNumberOfSupportedStreams (st)), 0, GetChannelWidth (st), GetAggregation (st), false);   
}
...
ConstantRateWifiManager::DoGetRtsTxVector (WifiRemoteStation *st)
{
  // Yubing
  NS_LOG_FUNCTION (this << st);
  if (m_mcsControlMap.find(GetAddress (st)) == m_mcsControlMap.end()) {
  ...
  return WifiTxVector (m_mcsControlMap[GetAddress (st)], GetDefaultTxPowerLevel (), GetShortRetryCount (st), GetPreambleForTransmission (m_ctlMode, GetAddress (st)), ConvertGuardIntervalToNanoSeconds (m_ctlMode, GetShortGuardInterval (st), NanoSeconds (GetGuardInterval (st))), 1, 1, 0, GetChannelWidth (st), GetAggregation (st), false);
}
