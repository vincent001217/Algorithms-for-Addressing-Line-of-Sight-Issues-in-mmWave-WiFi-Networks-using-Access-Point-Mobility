...
InterferenceHelper::CalculateChunkSuccessRate (double snir, Time duration, WifiMode mode, WifiTxVector txVector) const
{
  ...
  double csr = m_errorRateModel->GetChunkSuccessRate (mode, txVector, snir, (uint64_t)nbits);
  results.first = csr;
  results.second = 10 * log10 (snir * 1.3803e-23 * 290.0 * txVector.GetChannelWidth () * 1000000 * 10) + 30;
  NS_LOG_FUNCTION (this << 10 * log10 (snir * 1.3803e-23 * 290.0 * txVector.GetChannelWidth () * 1000000 * 10) + 30);
  return results;
}
...
{
  NS_LOG_FUNCTION (this << event->GetRxPowerW ());
  std::pair<double,double> results;
  ...
  Time plcpPayloadStart = plcpTrainingSymbolsStart + WifiPhy::GetPlcpTrainingSymbolDuration (event->GetTxVector ()) + WifiPhy::GetPlcpSigBDuration (preamble); //packet start time + preamble + L-SIG + HT-SIG or SIG-A + Training + SIG-B
  ...
  while (ni->end () != j)
    {
      ...
      if (previous >= plcpPayloadStart)
        {
          results = CalculateChunkSuccessRate (CalculateSnr (powerW,
                                                          noiseInterferenceW,
  ...
  double per = 1 - psr;
  results.first = per;
  NS_LOG_FUNCTION (this << results.second);
  return results;
}
          ...
          if (current >= plcpPayloadStart)
            {
              psr *= CalculateChunkSuccessRate (CalculateSnr (powerW,
                                                ...
                                                mcsHeaderMode, event->GetTxVector ()).first;
              ...
              if (preamble == WIFI_PREAMBLE_VHT || preamble == WIFI_PREAMBLE_HE_SU)
                {
                  ...
                  psr *= CalculateChunkSuccessRate (CalculateSnr (powerW,
                                                    ...
                                                    headerMode, event->GetTxVector ()).first;
...
InterferenceHelper::CalculatePlcpPayloadSnrPer (Ptr<InterferenceHelper::Event> event)
{
   ...
   */
  std::pair<double,double> results;
  results = CalculatePlcpPayloadPer (event, &ni);
  double per = results.first;
  struct SnrPer snrPer;
  snrPer.rss = results.second;
  snrPer.snr = snr;
  snrPer.per = per;
  return snrPer;
}
