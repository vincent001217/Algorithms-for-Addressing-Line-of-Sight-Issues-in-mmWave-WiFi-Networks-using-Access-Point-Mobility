...
{
  m_pktIdentifier++;
  NS_LOG_FUNCTION (this << m_pktIdentifier << packet << GetAddress () << to << packet->GetSize() << to.IsGroup ());
  if (!IsAssociated ())
    {
      NotifyTxDrop (packet);
      TryToEnsureAssociated ();
  ...
  NS_LOG_FUNCTION (found << accessPeriodInfo.nextHopAddress << GetBssid () << to.IsGroup () << to.IsBroadcast ());
  if (found && accessPeriodInfo.nextHopAddress != GetBssid () && !to.IsGroup ())//&& !to.IsGroup ()
    {
      hdr.SetAddr1 (accessPeriodInfo.nextHopAddress);
      hdr.SetAddr2 (GetAddress ());
      hdr.SetAddr3 (GetBssid ());
      hdr.SetDsNotTo ();
      //NS_LOG_FUNCTION (this << "1" << hdr.GetAddr1 ());
    }
  else
    {
      if (!to.IsBroadcast () && to.IsGroup ())
        {
          hdr.SetAddr1 (to);
          hdr.SetAddr2 (GetAddress ());
          hdr.SetAddr3 (GetBssid ());
          hdr.SetDsTo ();
          //NS_LOG_FUNCTION (this << "test 1" << hdr.GetAddr1 ());
        }
      else
        {     
        ...
        hdr.SetAddr3 (to);
        hdr.SetDsTo ();
        //NS_LOG_FUNCTION (this << "2" << hdr.GetAddr1 ());
        }
   } 
  hdr.SetDsNotFrom ();
  ...
  if (isCbap)
    {
      NS_LOG_FUNCTION (this << "edca" << hdr.GetAddr1 () << "header" << &hdr);
...
{
  NS_LOG_FUNCTION (this << "DMG STA Ending BI at " << Simulator::Now ());
          ...
          if (m_accessPeriod == CHANNEL_ACCESS_ABFT)
          ...
          else
            {
              ...
              if (m_isBeamformingInitiator)
                {
                  Simulator::Schedule (m_sbifs, &DmgStaWifiMac::SendInitiatorTransmitSectorSweepFrame, this, hdr.GetAddr1 (),
                                       m_sectorId, m_antennaId, m_totalSectors);
                }
              else
                {
...
{
  NS_LOG_FUNCTION (this << packet << hdr << hdr->GetAddr1 () << hdr->GetAddr2 ());
          ...
          if (!m_receivedDmgBeacon)
            {
              ...
              NS_LOG_LOGIC ("Good Beacon 2 " << !beaconInterval.IsCCPresent () << " " << !beaconInterval.IsDiscoveryMode () << " " << (int) m_nextAbft);
